...
describe("Will test crypto RSA ",function(){
    ...
    var public=fs.readFileSync(path.join(__dirname,"pubkey.pem")),
        public_ssh=fs.readFileSync(path.join(__dirname,"pubkey.ssh2")),
    ...
    it("Will test the node API, sign with node verify with crypto-browserify",function(){
        var signer=crypto.createSign("RSA-SHA1"),
            verifier=browserCrypto.createVerify("RSA-SHA1"),
            s, v,  msg="hola mundo";
            ...
            verifier.update(msg);
            v=verifier.verify(public_ssh,s);
            expect(v).to.be.true;
    });

    it("Will test the node API, sign with crypto-browserify verify with node",function(){
        var signer=browserCrypto.createSign("RSA-SHA1"),
            verifier=crypto.createVerify("RSA-SHA1"),
            s, v,  msg="hola mundo";
